...
func startSampler(s *store, every time.Duration) {
	go func() {
		// prime cpu percent (otherwise first read is often 0)
...
func readSnapshot(interval time.Duration) snapshot {
	...
	// Processes: best-effort, keep it bounded.
	ctx, cancel := context.WithTimeout(context.Background(), 800*time.Millisecond)
	defer cancel()

	if ps, err := process.ProcessesWithContext(ctx); err == nil {
		out.Procs = collectTopProcs(ctx, ps, 25)
	}
...
func collectTopProcs(ctx context.Context, ps []*process.Process, limit int) []procStat {
	...
	for _, p := range ps {
		...
		// These calls are relatively expensive; bail out if we’re out of time.
		select {
		case <-ctx.Done():
			goto done
		default:
